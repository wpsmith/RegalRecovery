...
@Suite("QuadrantWeeklyReviewScoringService Tests")
struct QuadrantWeeklyReviewScoringServiceTests {

    // MARK: - Balance Score

    @Test("high mean, low variance yields score >= 80")
    func testQuadrant_AC4_1_BalanceScoreHighMeanLowVariance() {
        let score = QuadrantWeeklyReviewScoringService.balanceScore(body: 8, mind: 8, heart: 8, spirit: 8)
        #expect(score >= 80.0)
    }
    ...
    func testQuadrant_AC4_2_BalanceScorePenalizesVariance() {
        let uniform = QuadrantWeeklyReviewScoringService.balanceScore(body: 8, mind: 8, heart: 8, spirit: 8)
        let variant = QuadrantWeeklyReviewScoringService.balanceScore(body: 10, mind: 10, heart: 10, spirit: 2)
        #expect(variant < uniform)
    }

    @Test("perfect scores of 10 yield 100.0")
    func testQuadrant_AC4_4_PerfectScoreIs100() {
        let score = QuadrantWeeklyReviewScoringService.balanceScore(body: 10, mind: 10, heart: 10, spirit: 10)
        #expect(score == 100.0)
    }

    @Test("all-ones score is positive due to zero variance")
    func testQuadrant_AC4_5_AllOnesIsMinimum() {
        let score = QuadrantWeeklyReviewScoringService.balanceScore(body: 1, mind: 1, heart: 1, spirit: 1)
        #expect(score > 0)
    }
    ...
    func testQuadrant_AC4_6_FlourishingRequiresHighMeanAndLowVariance() {
        let level = QuadrantWeeklyReviewScoringService.wellnessLevel(body: 8, mind: 8, heart: 9, spirit: 9)
    ...
    func testQuadrant_AC4_7_HighMeanHighVarianceIsNotFlourishing() {
        let level = QuadrantWeeklyReviewScoringService.wellnessLevel(body: 10, mind: 10, heart: 10, spirit: 2)
        #expect(level != .flourishing)
    }

    @Test("mean in 6-7 range yields growing")
    func testQuadrant_AC4_8_GrowingLevel() {
        let level = QuadrantWeeklyReviewScoringService.wellnessLevel(body: 6, mind: 7, heart: 6, spirit: 7)
        #expect(level == .growing)
    }

    @Test("mean in 4-5 range yields rebuilding")
    func testQuadrant_AC4_9_RebuildingLevel() {
        let level = QuadrantWeeklyReviewScoringService.wellnessLevel(body: 4, mind: 5, heart: 4, spirit: 5)
        #expect(level == .rebuilding)
    }

    @Test("mean below 4 yields struggling")
    func testQuadrant_AC4_10_StrugglingLevel() {
        let level = QuadrantWeeklyReviewScoringService.wellnessLevel(body: 2, mind: 3, heart: 2, spirit: 3)
    ...
    func testQuadrant_AC4_11_DetectsImbalancedQuadrant() {
        let imbalances = QuadrantWeeklyReviewScoringService.detectImbalances(body: 3, mind: 8, heart: 7, spirit: 8)
        #expect(imbalances == [.body])
    }

    @Test("uniform scores produce no imbalances")
    func testQuadrant_AC4_12_NoImbalanceWhenBalanced() {
        let imbalances = QuadrantWeeklyReviewScoringService.detectImbalances(body: 7, mind: 7, heart: 7, spirit: 7)
    ...
    func testQuadrant_AC4_13_MultipleImbalances() {
        let imbalances = QuadrantWeeklyReviewScoringService.detectImbalances(body: 2, mind: 9, heart: 2, spirit: 9)
    ...
    func testQuadrant_AC4_14_ThresholdExactly3() {
        ...
        let imbalances = QuadrantWeeklyReviewScoringService.detectImbalances(body: 4, mind: 7, heart: 7, spirit: 7)
        #expect(imbalances == [.body])
    }
    ...
    func testQuadrant_AC4_15_ThresholdJustBelow3() {
        ...
        let imbalances = QuadrantWeeklyReviewScoringService.detectImbalances(body: 5, mind: 7, heart: 7, spirit: 8)
        #expect(imbalances.isEmpty)
    }
    ...
    func testQuadrant_AC2_3_WeekStartDateIsMonday() {
        ...
        let weekStart = QuadrantWeeklyReviewScoringService.weekStartDate(for: wednesday)
    ...
    func testQuadrant_AC2_4_ISOWeekNumberCalculation() {
        ...
        let (weekNumber, year) = QuadrantWeeklyReviewScoringService.isoWeekComponents(for: date)
