...
func TestTimeJournal_TJ003_ActivityFreeText(t *testing.T) {
	...
	if entry.Activity == nil || *entry.Activity == "" {
...
// slots shows in-progress status.
//
// Acceptance Criterion (TJ-060): Default status with no overdue slots is
// in-progress. At 8AM with slots 0-7 filled and slot 8 (current) empty,
...
func TestTimeJournal_TJ030_StreakCalculation(t *testing.T) {
	...
	// Days must be sorted descending (most recent first) for CalculateStreak.
	days := []timejournal.TimeJournalDay{
		{Date: "2026-04-06", CompletionScore: 0.92},
		{Date: "2026-04-05", CompletionScore: 0.88},
		{Date: "2026-04-04", CompletionScore: 0.95},
		{Date: "2026-04-03", CompletionScore: 0.79}, // breaks streak
		{Date: "2026-04-02", CompletionScore: 0.85},
		{Date: "2026-04-01", CompletionScore: 0.90},
	}
...
// shows overdue (elapsed unfilled slots since slots 0-7 have elapsed).
...
// morning slots by noon shows in-progress (afternoon slots not yet elapsed).
...
func TestTimeJournal_StatusEngine_T30Mode(t *testing.T) {
	...
	// Slots 0-23 (midnight to noon) should be filled for in-progress at noon.
	now := time.Date(2026, 4, 6, 12, 0, 0, 0, time.UTC)
	entries := makeT30Entries(0, 23) // 24 half-hour slots filled (midnight-noon)
...
// slots filled, if the final slot hasn't elapsed yet, status is in-progress.
func TestTimeJournal_StatusEngine_FinalSlotNotElapsed(t *testing.T) {
	...
	// Then -- final slot not elapsed, so day is still in-progress
	if status != timejournal.StatusInProgress {
...
func TestTimeJournal_Streak_ConsecutiveDays(t *testing.T) {
	...
	days := []timejournal.TimeJournalDay{
		{Date: "2026-04-06", CompletionScore: 0.88},
		{Date: "2026-04-05", CompletionScore: 0.95},
		{Date: "2026-04-04", CompletionScore: 0.80},
		{Date: "2026-04-03", CompletionScore: 0.90},
		{Date: "2026-04-02", CompletionScore: 0.85},
	}
...
func TestTimeJournal_Streak_BrokenByLowCompletion(t *testing.T) {
	// Days sorted descending (most recent first).
	days := []timejournal.TimeJournalDay{
		{Date: "2026-04-06", CompletionScore: 0.95},
		{Date: "2026-04-05", CompletionScore: 0.79}, // breaks streak
		{Date: "2026-04-04", CompletionScore: 0.85},
		{Date: "2026-04-03", CompletionScore: 0.90},
	}
...
func TestTimeJournal_Streak_LongestVsCurrent(t *testing.T) {
	...
	days := []timejournal.TimeJournalDay{
		{Date: "2026-04-06", CompletionScore: 0.88},
		{Date: "2026-04-05", CompletionScore: 0.90},
		{Date: "2026-04-04", CompletionScore: 0.85},
		{Date: "2026-04-03", CompletionScore: 0.50}, // breaks streak
		{Date: "2026-04-02", CompletionScore: 0.92},
		{Date: "2026-04-01", CompletionScore: 0.88},
		{Date: "2026-03-31", CompletionScore: 0.90},
		{Date: "2026-03-30", CompletionScore: 0.85},
	}
...
func TestTimeJournal_Streak_ExactThreshold(t *testing.T) {
	days := []timejournal.TimeJournalDay{
		{Date: "2026-04-06", CompletionScore: 0.80}, // exactly at threshold
		{Date: "2026-04-05", CompletionScore: 0.80}, // exactly at threshold
	}
...
func TestTimeJournal_Streak_BelowThreshold(t *testing.T) {
	// Days sorted descending (most recent first).
	days := []timejournal.TimeJournalDay{
		{Date: "2026-04-06", CompletionScore: 0.85},
		{Date: "2026-04-05", CompletionScore: 0.79}, // just below threshold
		{Date: "2026-04-04", CompletionScore: 0.85},
	}
...
// testDate is the reference date used for all test entries.
const testDate = "2026-04-06"

// makeEntry creates a slice with a single TimeJournalEntry for the given T-60 slot index.
...
// makeEntries creates TimeJournalEntry values for T-60 slot indices from startIdx
// to endIdx (inclusive).
...
// makeT30Entries creates TimeJournalEntry values for T-30 slot indices from startIdx
// to endIdx (inclusive).
func makeT30Entries(startIdx, endIdx int) []timejournal.TimeJournalEntry {
	activity := "Test activity"
	entries := make([]timejournal.TimeJournalEntry, 0, endIdx-startIdx+1)
	for i := startIdx; i <= endIdx; i++ {
		hour := (i * 30) / 60
		...
		entries = append(entries, timejournal.TimeJournalEntry{
			...
			SlotEnd:   fmt.Sprintf("%02d:%02d:00", (hour*60+minute+30)/60, (minute+30)%60),
			Activity:  &activity,
			Mode:      timejournal.ModeT30,
			CreatedAt: time.Now(),
		})
